...
const ShortWordsGuide: React.FC = () => (
    <div className="mt-2 bg-slate-900/40 border border-slate-700 rounded-xl p-4 text-xs text-brand-subtle space-y-3 animate-fade-in">
        ...
        <div>
            <p className="font-semibold text-slate-300 mb-1">Palabras de 2 letras</p>
            <ul className="space-y-1">
                {TWO_LETTER_GROUPS.map(g => (
                    <li key={g.label}>
                        <span className="text-slate-500">{g.label}:</span>{' '}
                        <span className="font-mono text-brand-text">{g.words}</span>
                    </li>
                ))}
            </ul>
        </div>
        <div>
            <p className="font-semibold text-slate-300 mb-1">Palabras de 3 letras</p>
            <ul className="space-y-1">
